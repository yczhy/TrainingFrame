                ...
                void scene()
                {
                    ...
                    void mapIidsToDatas()
                    {
                        for (int i = 0; i < iids.Length; i++)
                            if (iids[i] == 0)
                                deletedGids.Add(gids[i].ToString());
                            else
                                sceneData.goDatasByInstanceId[iids[i]] = sceneData.goDatasByGlobalId[gids[i].ToString()];

                    }
                    void removeDeletedGids()
                    {
